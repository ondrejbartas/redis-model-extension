  ...
  context "Dynamic alias" do
    ...
    context "class" do    
      context "redis key" do
        ...
        should "validate redis_alias_key" do
          assert DynamicAlias.valid_dynamic_key?(:items_with_name, @args), "full valid args"
          assert !DynamicAlias.valid_dynamic_key?(:items_with_name, :name => "Test"), "only main fields"
          assert !DynamicAlias.valid_dynamic_key?(:items_with_name, :items_order => [:foobar]), "only fields order"
          assert !DynamicAlias.valid_dynamic_key?(:items_with_name, :items_order => [:foobar], :items => {foobar: "test"}), "only fields order & items"
          assert !DynamicAlias.valid_dynamic_key?(:items_with_name), "no args"
        end
    ...
    context "instance" do    
      context "redis key" do
        ...
        should "validate key" do
          ...
          assert_equal dyn_alias.valid_dynamic_key?(:items_with_name), true
          dyn_alias.items_order = nil
          assert_equal dyn_alias.valid_dynamic_key?(:items_with_name), false
        end
    ...
    context "saving & destroy" do
      setup do
        ...
        @redis_dynamic_alias_key = @dynamic_alias.redis_dynamic_key(:items_with_name)
      end
    ...
    context "find & get" do
      ...
      should "be getted by dynamic alias" do
        @getted_model = DynamicAlias.get_by_dynamic(:items_with_name, @args)
      ...
      should "be find all by dynamic alias" do
        @getted_models = DynamicAlias.find_by_dynamic(:items_with_name ,{})
        assert_equal @getted_models.size, 1, "Should be only one with alias" 
        @getted_model = @getted_models.first
        assert_equal @getted_model.name, @dynamic_alias.name
        assert_same_elements @getted_model.items.to_json.split(","), @dynamic_alias.items.to_json.split(",")
      ...
      should "be getted after change in alias" do
        ...
        assert_equal DynamicAlias.get_by_dynamic(:items_with_name ,@getted_model.to_arg).name, @getted_model.name
      end
    end
